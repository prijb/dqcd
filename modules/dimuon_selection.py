...
#SnT selections but for systematics (best chi2, then select)

class SnTMakeDimuonsRDFProducerSyst():
    def __init__(self, *args, **kwargs):
        ...
        //Gets the dimuons for the event (add a flag that asks whether it passed the selections or not)
        auto getDimuons(Vint Muon_ch, Vfloat Muon_pt, Vfloat Muon_eta, Vfloat Muon_phi, Vfloat Muon_phiCorr, 
        Vfloat Muon_dxyCorr, Vfloat Muon_dxye, Vbool Muon_selected,
        Vint Muon_bestAssocSVOverlapIdx, Vint Muon_bestAssocSVIdx, Vint Muon_nhitsbeforesv,
        float PV_x, float PV_y, float PV_z,
            ...
            for(unsigned i=0; i<Muon_pt.size(); i++){
                ...
                for(unsigned j=i+1; j<Muon_pt.size(); j++){
                    ...
                    if(same_vertex && is_neutral){
                        ...
                        if (!SV_selected.at(common_vertex)) pass_veto = 0;
                        if(material_veto) pass_veto = 0;
                        ...
                        //Angular cuts on SV
                        if(alpha3d_dimuon > (0.9*TMath::Pi())) pass_veto = 0;
                        if(dphi_dimuon > (0.9*TMath::Pi())) pass_veto = 0;
                        if(dphi > (TMath::Pi()/2.)) pass_veto = 0;
                        if(alpha3d > (TMath::Pi()/2.)) pass_veto = 0;
    ...
    def run(self, df):
        ...
        #Get the best vertex
        df = df.Filter("EventDimuons.mass.size() > 0")
        df = df.Define("DimuonBestIndex", "EventDimuons.bestvtxidx")
        df = df.Define("DimuonBestMass", "EventDimuons.mass[DimuonBestIndex]").Define("DimuonBestLxy", "EventDimuons.lxy[DimuonBestIndex]").Define("DimuonBestPass", "EventDimuons.passveto[DimuonBestIndex]")
        #Define the reweighting variables
        df = df.Define("DimuonBestSubleadingPt", "EventDimuons.subleading_pt[DimuonBestIndex]").Define("DimuonBestEta", "EventDimuons.dimuon_eta[DimuonBestIndex]")

        return df, ["DimuonBestMass", "DimuonBestLxy", "DimuonBestPass", "DimuonBestSubleadingPt", "DimuonBestEta"]

def SnTMakeDimuonsRDFSyst(*args, **kwargs):
    return lambda: SnTMakeDimuonsRDFProducerSyst(*args, **kwargs)